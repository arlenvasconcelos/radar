...
`view=trend` returns each series with `startHourlyCost`, `endHourlyCost` and `changePercent`, plus `trendTotal` summed across series per timestamp. `trendTotal.from`/`to` are the first and last sample timestamps (RFC3339), possibly shorter than the requested range; `sampleCount` counts timestamps. Rates are hourly, not cumulative spend. `changePercent` is explicitly null for zero starting cost or a single sample. Trend has no monthly projection.
