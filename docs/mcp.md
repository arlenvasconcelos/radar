...
| Basis | Meaning |
|-------|---------|
| `allocated` | `hourlyCost` equals `allocatedCost` and does **not** include `unallocatedCost`. Kubecost, and every scoped summary. |
| `node_capacity` | The unscoped OpenCost path raised `hourlyCost` to the summed node cost because nodes cost more than the allocations. It already contains `unallocatedCost` — do not add it again — and carries **no** storage, which node cost does not include. |
...
`view=trend` returns each series with `start`, `end` and `changePercent`, plus a top-level `total` of the same shape summed **across** series per timestamp, with `from`/`to` naming the first and last points — shorter than `range` when the source retains less history, and `changePercent` spans only that. Timestamps are RFC3339. Series values are hourly rates at each point, not cumulative spend, and `changePercent` is absent (not `0`) when the starting value was zero, because a change from nothing is undefined. Trend carries no `totals`, so `projectedMonthlyCost` does not apply to it. Only the highest-spending namespaces get their own series; the rest are summed into one named `other`. When that happens the response sets `truncated` with `namespaceCount`, and `total` still covers every namespace — the total is whole-scope even though the series are not.

`total.basis` names what the trend sums, which differs by source: `cpu_memory_allocation` (OpenCost via Prometheus — CPU and memory allocation only, so compare `total.end` with the summary's `allocatedCost` minus `storageCost`, never with `hourlyCost`) or `namespace_allocation` (Kubecost — the same components as `allocatedCost`, idle excluded).
...
| `get_cost` | Cluster spend from OpenCost (via Prometheus) or Kubecost (via its API) — currency, idle attribution, and the Kubecost/OpenCost source split are handled server-side. `view=summary` (default) returns cluster totals plus per-namespace rows; `view=workloads` breaks one namespace down, or returns one workload when given `kind` + `name`; `view=nodes` ranks node spend (cluster-wide; passing a namespace is an error); `view=trend` returns spend over time, pre-summarized. Spend, not usage — use `top_resources` for consumption and `get_rightsizing` for whether requests are sized right. Rates are hourly; totals also carry `projectedMonthlyCost` (hourly x 730, on the same `hourlyCostBasis`), matching the Costs UI. `available: false` carries `reason` + `remediation` rather than an error. Row counts (`namespaceCount` / `workloadCount` / `nodeCount`) accompany `truncated`. Every response explains its own fields in `guidance`; see [Cost and rightsizing evidence limits](#cost-and-rightsizing-evidence-limits) for `efficiency`, the two kinds of idle and `hourlyCostBasis`, node pools, trend basis, and scope attribution. | `view` (optional: `summary` default, `workloads`, `nodes`, `trend`), `namespace` (required for `workloads`; filters `summary`/`trend`), `kind` + `name` (optional, `view=workloads` only: target one workload), `range` (trend only: `6h`, `24h` default, `7d`), `limit` (optional, default 20, max 100) |
| `get_rightsizing` | Per-container CPU/memory request recommendations (current limits shown as context) from **7 days of observed usage** — not live metrics (use `top_resources` for those). Each row carries `fit` (`oversized`, `under_requested`, `missing_request`, `balanced`, `insufficient_history`) and a `confidence` tier; low confidence means short or sparse history, NOT correctly sized. `scope` is required and there is no default: `workload` is cheap, precise, and returns every row of that workload; `namespace` scans one namespace, or several with `namespaces` (excluded and empty names are reported, not dropped); `cluster` scans every Deployment/StatefulSet/DaemonSet with 7-day range queries and can take 45s — call it once to find candidates, then drill in with `scope=workload`. Scan scopes rank by `classification` then replica-weighted `impact` — the Rightsizing page's rule, applied per workload rather than per container, so the top-N can differ. Balanced containers are omitted unless `include_balanced=true`, except rows carrying OOM, limit-conflict, significant (≥10%) throttle or autoscaler evidence, which are always returned. `state=partial` always names its cause in `reason`, and `guidance` names the causes present. `hpaManaged`, `currentPodOOM`, and `windowOomEvidence` change what a safe recommendation is; `demandTarget` is context on clamped rows, never a value to apply, and `managedBy` says where a change belongs. See [Cost and rightsizing evidence limits](#cost-and-rightsizing-evidence-limits) for ranking, omission and partial-state detail. | `scope` (required: `workload`, `namespace`, `cluster`), `kind` + `name` (scope=workload), `namespace` (required for `workload`; for `namespace` unless `namespaces` is set; rejected for `cluster`), `namespaces` (optional, `scope=namespace` only: several namespaces in one call; the 45s budget is shared), `include_balanced` (optional, default false; ignored for `scope=workload`), `limit` (optional, default 20, max 100; rejected for `scope=workload`) |
