...
- **RBAC-aware** — every call enforces RBAC at the same boundary as the REST API:
  ...
  - **Headless clients**: an MCP client that can't run a browser login authenticates with a per-user API key (`Authorization: Bearer rk_...` or `X-Api-Key: rk_...`). The key carries the identity of the user who created it, so the RBAC rules above apply unchanged — see [API Keys](authentication.md#api-keys-headless-clients)
