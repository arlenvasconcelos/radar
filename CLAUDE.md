...
- GitOps nodes: Application (ArgoCD), Kustomization, HelmRelease, GitRepository (FluxCD)
  ...
  - **GitOps workflow operations**: Argo apps support Sync (with full options dialog — prune, dry-run, apply-only, force, replace, server-side apply, sync-options), Refresh, Hard refresh (`syncOptions: ["RefreshType=hard"]`), Terminate (running operations), Suspend/Resume auto-sync, Rollback by `status.history[].id`, Selective sync of subset of resources. Flux Kustomizations/HelmReleases support Reconcile, Suspend/Resume, Reconcile-with-source. Operations live in `pkg/gitops/operations.go` with sentinel errors (`ErrOperationInProgress`, `ErrNoOperationInProgress`, `ErrHistoryEntryNotFound`, `ErrResourceTerminating`); HTTP layer routes via `errors.Is`, never substring matching. Rollback options accept `Force`/`ApplyOnly` as `*bool` so omission ≠ false on the wire — important because Argo's `syncStrategy.apply.force=true` silently skips PreSync/PostSync hooks; Force-only routes via `syncStrategy.hook.force` to preserve hooks, ApplyOnly reserves `syncStrategy.apply`.
  - **Lifecycle (Terminating) handling**: every mutating operation (Sync, Reconcile, SetAutoSync, Rollback, SyncWithSource, SetSuspend) calls `assertNotTerminating` after its initial GET — a resource with `metadata.deletionTimestamp` set returns `ErrResourceTerminating` mapped to HTTP 409. Refresh and Terminate intentionally bypass this check (read/cleanup-only verbs). The same gate runs for MCP `manage_gitops` so direct API hits also fail with the typed error. Frontend reads `summary.terminating` to render an orange `[Terminating]` chip, suppress Sync/Health badges (factually stale once deletion is initiated), disable mutating action buttons with reason tooltips, and hoist the lifecycle Issue to a banner above the regular Issues panel with pre-deletion failures collapsed behind a "Pre-deletion issues (N)" disclosure. The fleet view replaces Sync/Health column values with `—`, paints the row stripe orange, and promotes the `[TERMINATING]` chip to the leftmost slot. Graph nodes for Terminating CRs render with an orange left-stripe and a single `Terminating` chip (suppressing sync/health chips). Cluster Audit gains a `stuckTerminating` check across all typed K8s resources with the same warning(5min)/danger(30min) thresholds as the per-resource Issue, so audit and insights agree on severity. Home dashboard's `GitOpsControllersCard` (in `internal/server/dashboard_gitops.go`) detects up to 8 controllers (3 Argo CD, 5 Flux) by static label-selector probe and surfaces "Controllers healthy/degraded/crashing" — capability-gated to nil when no controllers detected.
  - **Finalizer→controller catalog** (`pkg/gitops/insights/finalizers.go`): static map from finalizer key + resource group to (controller name, namespace, label selector). Used by `detectPendingDeletion` to enrich the lifecycle Issue's `Cause` field with concrete controller-health attribution ("argocd-application-controller is CrashLoopBackOff", "helm-controller is not running in namespace flux-system") via the Resolver's `FinalizerOwnerStatus` method. Update this catalog when Argo or Flux releases new controllers; missing entries degrade gracefully (Issue still emits; Cause stays empty).
  - **Nested GitOps navigation (portal nodes + lineage breadcrumb)**: `pkg/gitops/tree/graph.go::classifyGitOpsKind` tags every enriched node with `data.gitopsTool` (`"argocd" | "fluxcd"`) and `data.gitopsKind` when the kind is itself a GitOps detail-page CR (Argo Application/ApplicationSet/AppProject, Flux Kustomization/HelmRelease/source CRDs). Frontend renders these with a tool badge + skyhook ring + chevron and routes click → that CR's own detail page. The receiver page reads `?from=kind|namespace|name` to render an extra breadcrumb segment (`GitOps / parent-ns/parent / child-ns/child`) with the parent clickable to navigate back. The `from=` separator is a single pipe; namespaces or names containing `|` would lose precision (rare; current usage is GitOps-CR only).
  - **Severity vocabulary** for issues: `critical` (rank 0, red) → `alert` (rank 1, orange) → `warning` (rank 2, amber) → `info` (rank 3, blue). The `alert` tier was added so the lifecycle detector can ramp severity by deletion age (info <5min, warning 5-30min, alert >30min). `severityRank` knows about all four tiers; adding a new severity requires updating both the rank function in `pkg/gitops/insights/insights.go` and the matching `'critical' | 'alert' | 'warning' | 'info'` union in `packages/k8s-ui/src/types/gitops-insights.ts`.
  - **Insights diagnosis pipeline** (`pkg/gitops/insights/`): `Build` composes Summary + Issues + Changes + Plan + History + Capabilities. Issues come from a chain of detectors: `detectPendingDeletion` (lifecycle, severity-ramped by deletion age) → operation-failure parser (10 recognized patterns: annotation-too-long, admission-webhook, RBAC, conflict, immutable-field, schema-migration, hook-failure, connectivity, etc.) → `detectStuckDriftLoop` (sync=OutOfSync + opPhase=Succeeded + auto-sync on + reconciledAt<30min → "controller mutating after each apply") → `detectManualDriftWithoutAutoSync` (drift exists, no auto-sync → "click Sync") → `argoApplicationConditions` (extracts status.conditions[] with type-aware severity) → per-resource degraded/missing/OutOfSync issues (suppressed when ref matches the operation failure's affected resource — no triplicate rendering). Output is deduped by `(scope, reason, message, firstRef)` so Flux's same-message-twice condition pattern collapses to one row. Per-resource drift comes from `computeDriftFromLastApplied` which parses the `kubectl.kubernetes.io/last-applied-configuration` annotation and recursively diffs against live spec; works for any Argo client-side-applied resource. Recent events come from the Resolver interface wiring the cache's event lister with namespace-RBAC filtering and a 5-event cap. The Resolver also exposes `FinalizerOwnerStatus` for the lifecycle detector. Insight refetches at 2s polling cadence while operationPhase is Running, otherwise on-demand. Issue order in the array follows severity-rank (critical → alert → warning → info), so a critical operation failure can land above an alert-tier lifecycle issue; the user-facing "lifecycle dominates" contract is enforced by the frontend `GitOpsIssuesBand` extracting scope=lifecycle to render as a banner above all other issues regardless of array position.
