...
**Kyverno demo cluster** (`scripts/kyverno-demo.sh` + `make kyverno-demo`): bootstraps a `kind` cluster with live Kyverno admission, both legacy and modern policy APIs, dual-shape PolicyExceptions, and real background PolicyReports across multiple producer strings. Use it for policy renderers, enforcement-posture work, report-family selection, and admission-denial attribution. `openreports` leaves `wgpolicyk8s.io` served but empty while reports land in `openreports.io`; `modern-only` removes the legacy CRDs and intentionally leaves Kyverno 1.18's admission controller crashlooping. Read `scripts/kyverno-demo/README.md` before using either scenario.

**Velero demo cluster** (`scripts/velero-demo.sh` + `make velero-demo`): all 13 Backup phases, the supersession series (a failed nightly superseded by a later success, plus an unlabelled ad-hoc failure), a paused-and-invalid Schedule, an unavailable BackupStorageLocation, a restic BackupRepository, and the rancher `restores.resources.cattle.io` plural collision. **The Velero controller is deliberately scaled to 0**: without object storage a live controller yields exactly one outcome, everything failed on credentials, so the fixtures carry their own status and nothing reconciles them away — which is what makes all 13 phases reachable at once. `./scripts/velero-demo.sh live` is the other half: it installs MinIO as in-cluster S3 and has Velero produce a real backup, a real restore of a deleted namespace, a real `PartiallyFailed` (a pre-backup hook that exits non-zero), a real `FailedValidation` (a backup aimed at a location Velero has marked `Unavailable`), a location it marks `Unavailable` itself, and a run left genuinely in flight. Use `live` whenever a claim needs to rest on something Velero decided rather than something a fixture asserts — a fixture can be written to say anything. The messages behind a run's error and warning counts are reachable in `live` (that is what the `messages` endpoint fetches); full backup logs, progress counters ticking mid-run, and data-mover CRs stay out of reach in both (final item counts are real in `live`). See `scripts/velero-demo/README.md`.

**CloudNativePG demo cluster** (`scripts/cnpg-demo.sh` + `make cnpg-demo`): same rationale as the GitOps one, for CNPG. Four Postgres clusters that all reach 2/2 Ready and render four *different* badges (Healthy / WAL Archiving Failing / Unrecoverable ×2), plus a Pooler, a plugin-method ScheduledBackup, three Backups, and Velero + KubeBlocks CRs so the shared `backups`/`clusters` plural guards are actually exercised. Run `make cnpg-demo` and `kubectl config use-context kind-radar-cnpg-demo` before `./scripts/visual-test-start.sh`. **Read `scripts/cnpg-demo/README.md` before changing the fixtures** — freezing the operator does not hold a status (each Postgres pod's instance manager rewrites conditions within ~10 min), the WAL failure has to be caused rather than patched, and several of the ordering constraints fail in ways that look like success. `make cnpg-demo-live` leaves the operator running for work that needs real failovers.

**Beyla demo cluster** (`scripts/beyla-demo.sh` + `make beyla-demo`): Grafana Beyla with eBPF actually loaded on `kind`, a minimal Prometheus scraping it, and three conversations (HTTP, raw TCP, and DNS — the only UDP). Use it for anything touching `internal/traffic/beyla.go`, because **which labels Beyla exports depends on its configuration and cannot be read off the code**: `dst_port` and `transport` are `Default: false` and absent from a stock install, while `direction` is on by default and reports every conversation twice with source and destination swapped. `beyla-demo.sh attrs` switches to the configuration that selects the two optional attributes — which also makes the response-direction series carry ephemeral client ports, taking one conversation from 2 series to 400. `no-network` reproduces the stock default where the flow metric does not exist at all and only `beyla_build_info` remains. **Read `scripts/beyla-demo/README.md` before changing the Beyla source** — the three configurations are not interchangeable, and configuration A (defaults) is what users actually have.
