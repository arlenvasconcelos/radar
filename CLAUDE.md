...
- API keys: `/api/auth/api-keys` (GET list, POST create) + `/api/auth/api-keys/{id}` (DELETE). Registered only when `--auth-api-keys-file` resolves a store, which requires `--auth-mode` != `none` and **not** Radar Cloud (the middleware ignores keys on tunneled requests, so `openAPIKeyStore` returns nil under `cloud.Mode()` rather than registering routes that mint credentials which can never authenticate — the Hub issues its own tokens). A `Lookup` error fails closed with 503, never falling through to the proxy-header identity. Every operation is scoped to the caller — another user's key is a 404, never a 403. The plaintext is returned once by POST and only `hex(sha256())` is persisted; a key authenticates via `Authorization: Bearer rk_...` or `X-Api-Key` and resolves to its creator's username+groups, so RBAC/impersonation are unchanged. POST/DELETE additionally require a same-origin request (`sameOriginOnly`): the CORS allowlist admits any localhost origin with credentials and `SameSite=Lax` treats localhost ports as same-site, so without it any local page could mint a key and read the plaintext. Store in `internal/auth/apikeys.go` (SQLite — **not** `pkg/auth`, which is a separate module kept free of a DB driver), handlers in `internal/auth/apikey_handlers.go`, middleware branch between the cookie and proxy-header blocks. Cross-owner list/revoke lives **only** in the `radar api-keys` CLI (`cmd/explorer/apikeys_cmd.go`) — `ListAll`/`DeleteByID`/`DeleteAllForUser`/`DeleteAll` are deliberately off the `APIKeyStore` interface so no HTTP handler can reach them; authorization there is filesystem access to the database (Radar has no admin identity), and it is the only way to revoke a departed user's keys. In-cluster the database needs the chart's PVC (`persistence.enabled` + `auth.apiKeys.persist`, mounted at `/data`) — `~/.radar` is an `emptyDir`, so the default path loses every key on pod restart. SQLite is single-writer, so the chart hard-fails `replicaCount > 1` whenever that volume is in use
